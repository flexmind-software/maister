...
UI mockups are generated by the shared **`mockup-studio`** skill, used by both the development workflow (Phase 4) and the product-design workflow (Phase 7), and invocable standalone (`/maister:mockup-studio`). It runs in the main agent context, so it can drive a live visual-companion server, a browser preview, and interactive refinement gates.
...
- Standalone — `/maister:mockup-studio "<screen or feature>"`
